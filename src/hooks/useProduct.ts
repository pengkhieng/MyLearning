...
export const useProducts = () => {
  ...
  const addProduct = useCallback(
    async (
      product: { name: string; description: string; price: number; categoryId: string; stock: number; active?: boolean },
      imageUrl?: string
    ) => {
      setLoading(true);
      setError(null);
      ...
      try {
        ...
        let url = ApiEndpoints.PRODUCT.CREATE;
        if (imageUrl) {
  ...
  const editProduct = useCallback(
    async (
      ...
      try {
        ...
        let url = `${ApiEndpoints.PRODUCT.UPDATE}/${id}`;
        if (imageUrl) {
